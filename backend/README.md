...
Local run is handled by Vercel (serverless). For local testing, use:
...
This backend is prepared for Vercel serverless functions using a single `index.go` file (`package handler`).
